...
RC initBufferPool(BM_BufferPool * const bm, const char * const pageFileName,
	...
	intialize_linked_list(bm->numPages);

	display_linked_list(bm->numPages);
			...
			if (frameToUnpinPage->data == pageNum){
				...
				if (frameToUnpinPage->dirty_bit) {
					...
					//int status = forcePage(bm, page);
				        printf("BEFORE::Number Of Writes:-%d\n",bm_statistics->numWriteIO);
					writeBlock1(page->pageNum,&fHandle,(SM_PageHandle)page->data);

	 				bm_statistics->numWriteIO++;
	 				printf("AFTER::Number Of Writes:-%d\n",bm_statistics->numWriteIO);
...
PageNumber *getFrameContents(BM_BufferPool * const bm) {
	printf("In getFrameContents Method!!!");
